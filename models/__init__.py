...
from models.img_resnet import ResNet50, DomainAug, FeatAug
...
def build_model(config, num_identities, num_clothes):
    ...
    domain_aug = DomainAug(config.MODEL.FEATURE_DIM, num_identities)
    feat_aug = FeatAug(probability=config.probability)

    return model, domain_aug, feat_aug, identity_classifier, clothes_classifier
