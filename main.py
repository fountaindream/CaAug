...
def parse_option():
    ...
    parser.add_argument('--aug_loss_weight', default=1.0, type=float, help='loss weight for aug_loss')    
    parser.add_argument('--p', default=0.2, type=float, help='probability')  
...
def main(config):
    ...
    # Build model
    model, domain_aug, feat_aug, classifier, clothes_classifier = build_model(config, dataset.num_train_pids, dataset.num_train_clothes)
    model_aug, domain_aug_, _, classifier_aug, _ = build_model(config, dataset.num_train_pids, dataset.num_train_clothes)
    # Build identity classification loss, pairwise loss, clothes classificaiton loss, and adversarial loss, aug_loss.
    criterion_cla, criterion_pair, criterion_clothes, criterion_adv = build_losses(config, dataset.num_train_clothes)
    # Build optimizer
    parameters = list(model.parameters()) + list(classifier.parameters()) + list(domain_aug.parameters()) + list(feat_aug.parameters())
    ...
    if config.MODEL.RESUME:
        ...
        domain_aug.load_state_dict(checkpoint['domain_aug_state_dict'])  
        feat_aug.load_state_dict(checkpoint['feat_aug_state_dict'])      
    ...
    domain_aug = domain_aug.cuda(local_rank)
    feat_aug = feat_aug.cuda(local_rank)
    model_aug = model_aug.cuda(local_rank)
    domain_aug_ = domain_aug_.cuda(local_rank)    
    classifier_aug = classifier_aug.cuda(local_rank)    
    if config.LOSS.CAL == 'calwithmemory':
    ...
    if config.TRAIN.AMP:
        [model, classifier, domain_aug, feat_aug], optimizer = amp.initialize([model, classifier, domain_aug, feat_aug], optimizer, opt_level="O1")
        if config.LOSS.CAL != 'calwithmemory':
    ...
    domain_aug = nn.parallel.DistributedDataParallel(domain_aug, device_ids=[local_rank], output_device=local_rank)   
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        if config.LOSS.CAL == 'calwithmemory':
            ...
            train_cal(config, epoch, model, domain_aug, feat_aug, classifier, model_aug, domain_aug_, clothes_classifier, criterion_cla, criterion_pair, 
        ...
        if (epoch+1) > config.TEST.START_EVAL and config.TEST.EVAL_STEP > 0 and \
            ...
            model_aug_state_dict = model_aug.state_dict()
            classifier_state_dict = classifier.module.state_dict()
            domain_aug_state_dict = domain_aug.module.state_dict()
            feat_aug_state_dict = feat_aug.state_dict()
            if config.LOSS.CAL == 'calwithmemory':
            ...
            if local_rank == 0:
                save_checkpoint({
                    ...
                    'model_aug_state_dict': model_aug_state_dict,
                    'classifier_state_dict': classifier_state_dict,
                    'clothes_classifier_state_dict': clothes_classifier_state_dict,
                    'domain_aug_state_dict': domain_aug_state_dict,
                    'feat_aug_state_dict': feat_aug_state_dict,
                    'rank1': rank1,
                    'epoch': epoch,
